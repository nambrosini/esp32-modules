...
use esp_hal::main;
use esp_hal::rmt::{CHANNEL_RAM_SIZE, PulseCode, Rmt, RxChannelConfig, RxChannelCreator};
...
fn main() -> ! {
    ...
    let mut channel = rmt
        .channel2
        .configure(peripherals.GPIO5, rx_config)
        .unwrap();
        ...
        match channel.receive(&mut buffer) {
            Ok(()) => {
                if let Some((address, command)) = decode_nec_samsung(&buffer) {
                    ...
                    println!("Frame ricevuto ma non riconosciuto (rumore o protocollo diverso)");
                }
            }
            Err(_e) => {
                // timeout: nessun tasto premuto, normale
            }
        }
